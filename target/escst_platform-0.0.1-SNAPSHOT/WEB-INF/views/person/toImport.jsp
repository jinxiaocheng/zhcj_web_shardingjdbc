...
<body>
	<div class="response-head titlePanel form-inline">
        <div class="ui-filter area" style="display: inline-block;margin:5px 0 0 10px ">
            <label>区域:</label>
        ...
        <div class="ui-filter site" style="display: inline-block;margin:5px 0 0 10px">
            <label>工地:</label>
            <div class="btn-group queryCondition">
                <a class="btn btn-default dropdown-text" data-toggle="dropdown" aria-expanded="false"
      ...
      <script>
          $(function(){
              $("#do_import_btn").on("click",function(){
                  ...
                  if (constructionId == "") {
                      layer.msg('请选择工地！', {time: 2000, icon: 0});
                      return;
                  }
                  if (file == "") {
                  ...
                  $.ajaxFileUpload({
                      ...
                      success: function (data, status) {    //提交成功后自动执行的处理函数，参数data就是服务器返回的数据
                          data = $.parseJSON(data);
                          if(data.status == 1){
                              layer.alert(data.value,function(){
                                  $(".my_load").hide();
                                  layer_close();
                              });

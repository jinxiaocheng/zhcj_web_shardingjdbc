...
<div class="ui-report">
    ...
    <div class="form-inline response-head titlePanel" id="listTools" >
        <div class="ui-filter">
            <label>角色名称:</label>
            <input type="text" id="name" placeholder="定义您的角色名称" class="form-control w-110" value="${name}"/>
        </div>
        <div class="ui-filter">
            <label>所属机构:</label>
            <input type="text" id="org" readonly class="form-control w-110" value="${orgName}"/>
        </div>
    </div>
...
<script type="text/javascript">
    $(function () {
        ...
        user.sendData({
            url: ctx + "/role/edit",
            data:function (menuAuthMap) {
                return {
                    roleId:roleId,
                    orgId: orgId,
                    name: $("#name").val(),
                    menuAuthMap: JSON.stringify(menuAuthMap)
                }
            },
            successText:'编辑角色成功',
            refreshUrl: ctx + "/role/roleList"
        });
    })
</script>
</body>

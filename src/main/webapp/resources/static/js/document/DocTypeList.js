function createTable(type) {
    ...
    $('#treeTable').treegrid({
        url: ctx + "/document/selectDocType",
        method: 'post',
        lines: true,
        idField: 'id',
        treeField: 'name',
...
var queryParams=function () {
    ...
    return {
        ...
        name:$("#name").val(),
        type:$(".container-full").attr("data-value"),
        rowNum: pageNum
    };
};
...
function addEdit() {
    ...
    if(type==0){
        nodeId=uploadData.parentId=$("#add-edit").attr("data-value");
        url=ctx+'/document/saveDocType';
        successText='新增成功';
    }else if(type==1){
        uploadData.id=$("#add-edit").attr("data-value").split('*')[1];
        nodeId=uploadData.parentId=$("#add-edit").attr("data-value").split('*')[0];
        url=ctx+'/document/saveDocType';
        successText='编辑成功';
    }
...
$("#delete_link").click(function () {
    ...
    layer.confirm('您是否要删除该文档类型？', {
        btn: ['是','否'] //按钮
    }, function(){
        var index=layer.load();
        $.ajax({
            type: "post",
            url: ctx + '/document/deleteDocType',
            data:{
                id:node.id,
                "constructionId": constructionId
            },

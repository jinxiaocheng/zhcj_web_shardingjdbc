...
function clickStartPlayback() {
    var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex), szIP = $("#ip")
        ...
        "#transstream").attr("checked"), iRet = -1;
...
//	alert(szIP);
    ...
    // 窗口分割 3x3
    if ($("#channels option").length > 9) {
        changeWndNum(4);
    } else if ($("#channels option").length > 4) {
        changeWndNum(3);
    } else if ($("#channels option").length > 1) {
        changeWndNum(2);
    } else if ($("#channels option").length == 1) {
        changeWndNum(1);
    }
    ...
    $("#channels option").each(function(i) {
        ...
        var iRet = WebVideoCtrl.I_StartRealPlay(szIP, {
            iWndIndex : i,
            iStreamType : iStreamType,
            iChannelID : parseInt($(this).val(),10),
            bZeroChannel : bZeroChannel
        });
    });
}

...
<div class="ui-report">
    <div class="response-head titlePanel form-inline">
        <div class="ui-filter">
            ...
            <label>型号名称:</label>
            <input type="text" id="model" name='model' class="form-control w-110" value=""/>
        </div>
...
<script type="text/javascript">
    ...
    function getGridHead() {
        return [
            ...
            {'label': '材料名称', 'name': 'materialName', 'index': 'name'},
            {'label': '规则型号', 'name': 'modelName', 'index': 'model'},
            {'label': '计量单位', 'name': 'unit', 'index': 'unit','width':'80px'},
            {'label': '数量', 'name': 'qty', 'index': 'quantity','width':'80px'},
            {'label': '生产厂家', 'name': 'manufacturer', 'index': 'manufacturer','width':'200px'},
            {'label': '使用部位', 'name': 'usePosition', 'index': 'usePosition'}
        ];
    };
    // 点击查询，获取查询所需要的条件，重新加载表格
    function getGridParamJson() {
        var constructionId=window.location.search.split("=")[1]
        //人员名称
        var postPrams = {
    ...
    //获取Url
    function getGridUrl() {
        return ctx + "/materialAcquisition/queryMaterialAvailableList";
    };
